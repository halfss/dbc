...
def sync(state):
    ...
    if max_id > state.block['index']:
        print "%s(%s) is larger than %s(local max)" % (max_id, longest_peer, state.block['index'])
        sync_block_range(longest_peer, state.block['index'], max_id)
...
def sync_block_range(peer, start, end):
    for i in range(start+1, end+1):
        block_json = requests.get("%s/block/%s" % (peer, i)).json()
        block = Block(block_json['index'], block_json['timestamp'], block_json['data'], block_json['previous_hash'])
        block.save()
